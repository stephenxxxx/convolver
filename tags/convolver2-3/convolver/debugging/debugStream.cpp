...
apDebugSinkConsole apDebugSinkConsole::sOnly = apDebugSinkConsole ();

apDebugSinkConsole::apDebugSinkConsole  () {}
apDebugSinkConsole::~apDebugSinkConsole () { flush();}


void apDebugSinkConsole::write (const std::string& str)
{
  buffer_ += str;
  flush ();
}

void apDebugSinkConsole::write (int c)
{
  buffer_ += (char)c;
  if (c == '\n')
    flush ();
...
apDebugSinkFile apDebugSinkFile::sOnly = apDebugSinkFile ();

apDebugSinkFile::apDebugSinkFile  () {}
apDebugSinkFile::~apDebugSinkFile () { flush();}

void apDebugSinkFile::setFile (const std::string& file)
{
  flush(); 
  file_ = file;
}
...
{
  OutputDebugString (CA2CT(str.c_str()));
}
#endif
